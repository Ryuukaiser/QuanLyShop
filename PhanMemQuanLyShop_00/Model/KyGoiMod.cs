...
namespace PhanMemQuanLyShop_00.Model
{
    class KyGoiMod
    {
        ...
        public void MoKetNoi()
        {
            try
            {
                if (conn == null)
                {
                    ...
                    conn = new SqlConnection(@"Data Source=.\SQLEXPRESS;Initial Catalog=ShopChoMeo;Integrated Security=True");
                }
        ...
        {
            string sqlSua = "INSERT INTO [ShopChoMeo].[dbo].[ThongTinKyGoi] ([MaThongTinKyGui],[TenChu],[LienHe],[CMND],[TenThuCung],[SoLuong],[NgayGoi],[NgayTra],[GiaComBo],[MaNhanVien],[TenChuong],[GiayTo]) VALUES ('" + maThongTinKyGui + "',N'" + tenChu + "',N'" + lienHe + "','" + cMND + "',N'" + tenThuCung + "','" + soLuong + "',CONVERT(datetime, '" + ngayGoi + "', 111),CONVERT(datetime, '" + ngayTra + "', 111),'" + giaComBo + "','" + maNhanVien + "','" + tenChuong + "',N'" + giayTo + "')";
            bool kt = false;
            if (ExecuteNonQuery(sqlSua) > 0)
            {
                kt = true;
        ...
        {
            string sqlSua = "INSERT INTO [ShopChoMeo].[dbo].[ThanhToanKyGoi] ([MaThanhToanKG] ,[ThanhTien] ,[MaNhanVien] ,[TienPhat] ,[GiaComBoTT] ,[NgayLap] ,[MaChuong] ,[MaThongTinKyGoi] ,[MaNVThanhToan]) VALUES ('" + maThanhToan + "' ,'" + thanhTien + "' ,'" + maNhanVien + "' ,'" + tienPhat + "' ,'" + giaCombo + "' ,CONVERT(datetim, '" + ngayLap + "', 111) ,'" + maChuong + "' ,'" + maThongTinKyGoi + "' ,'" + maNVThanhToan + "') ";
            bool kt = false;
            if (ExecuteNonQuery(sqlSua) > 0)
            {
                kt = true;
        ...
        {
            string sqlSua = "UPDATE [ShopChoMeo].[dbo].[ThongTinKyGoi] SET [MaThongTinKyGui] = '" + maThongTinKyGui + "',[TenChu] = N'" + tenChu + "',[LienHe] = N'" + lienHe + "',[CMND] = '" + cMND + "',[TenThuCung] = N'" + tenThuCung + "',[SoLuong] = '" + soLuong + "',[NgayGoi] = CONVERT(datetime, '" + ngayGoi + "', 111),[NgayTra] = CONVERT(datetime, '" + ngayTra + "', 111),[GiaComBo] = '" + giaComBo + "',[MaNhanVien] = '" + maNhanVien + "',[TenChuong] = '" + tenChuong + "',[GiayTo] = N'" + giayTo + "' WHERE [MaThongTinKyGui] = '" + maThongTinKyGui + "' ";
            bool kt = false;
            if (ExecuteNonQuery(sqlSua) > 0)
            {
                kt = true;
